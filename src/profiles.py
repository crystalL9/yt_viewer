...
import os
from src import watch_video
import threading
from src.module_tool import get_folder_profile_path,delete_profile
        ...
        if path.exists(profile_path):
            ...
            if flag!='0':
                ...
                try:
                    try:
                        ...
                        with open('data\logs.txt', 'a',encoding='utf-8') as file:
                            ...
                            file.write("-------------------\n")
                            file.close
                        with open('data\mail_error.txt', 'a',encoding='utf-8') as file:
                            file.write(f'{account}\n')
                            file.close
                        driver.quit()
                        return 0
                ...
                if not path.exists(profile_path):
                    ...
                    return 0
def create_profile_from_file_noproxy(queue,queue_position):
        account = queue.get()
        position=queue_position.get()
        username = account.split('|')[0]
        password = account.split('|')[1]
...
def process_mail(queue, use_proxy, num_threads):
    ...
    for i in range(num_threads):
         queue_position.put(f'{x}|{y}')
         x=x+20
    if use_proxy==0:
        for _ in range(num_threads):
            thread = threading.Thread(target=create_profile_from_file_noproxy, args=(queue,queue_position))
    ...
    while not queue.empty():
        ...
        if flag == '0' :
            ...
            if mode==4:
                threads = []
                for _ in range(len_profile):
                    thread = threading.Thread(target=manager_profiles_delete_history, args=(queue,queue_position))
                    threads.append(thread)
                    thread.start()
                for thread in threads:
                    thread.join()
            if mode==5:
                threads = []
                for _ in range(len_profile):
                    thread = threading.Thread(target=manager_profiles_watch_search_nora, args=(queue,keywords,start_time,end_time,queue_position))
                    threads.append(thread)
                    thread.start()
                for thread in threads:
                    thread.join()
            if mode==6:
                threads = []
                for _ in range(len_profile):
                    thread = threading.Thread(target=manager_profiles_watch_link_nora, args=(queue,links,start_time,end_time,queue_position))
                    threads.append(thread)
                    thread.start()
                for thread in threads:
                    thread.join()
...
def action_create(use_proxy,numthread):
    read_file_to_queue(file_path='data\gmail.txt',queue=queue_mail)
    default=queue_mail.qsize()
    queue_mail.queue.clear()
    queue_proxy.queue.clear()
    queue_profile.queue.clear()
    if numthread==0:
        numthread==default
    #refresh_()
    write_flag('1')
    path_mail='data\gmail.txt'
    ...
    print("Size start: ",queue_mail.qsize())
    while min_value > 0: 
        if use_proxy==0:
